...
@st.cache_data(show_spinner=False, ttl=3600)
def detail_columns(uf: str) -> set[str]:
    """Return the columns available in a state's detail Parquet file."""
    path = DETAIL_DIR / f"{uf}.parquet"
    source = parquet_source(path)
    with duckdb.connect(database=":memory:") as conn:
...
def detail_query(uf: str, where: str, limit: int, offset: int = 0) -> str:
    # Older processed files do not contain `titular`. Keep the table usable
    # during deployments where app.py is updated before all Parquets.
    titular_expression = (
        "titular" if "titular" in detail_columns(uf) else "CAST(NULL AS VARCHAR)"
    )
    return f"""
        SELECT
...
def show_table(uf: str, where: str, params: list[Any], total_rows: int) -> None:
    ...
    table_df = query_dataframe(
        uf, detail_query(uf, where, TABLE_PAGE_SIZE, offset), tuple(params)
    )
    st.caption(f"Página {int(page)} de {total_pages} · {total_rows:,} registros".replace(",", "."))
    st.dataframe(table_df, width="stretch", hide_index=True)

    if total_rows <= MAX_CSV_ROWS:
        csv_df = query_dataframe(uf, detail_query(uf, where, total_rows), tuple(params))
        st.download_button(
