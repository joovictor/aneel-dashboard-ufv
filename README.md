...
O projeto usa arquivos pré-processados a partir da relação pública de empreendimentos de geração distribuída da ANEEL, filtra empreendimentos fotovoltaicos (`SigTipoGeracao = UFV`) e publica um dashboard com indicadores, filtros, gráficos, mapa geográfico e tabela detalhada com coordenadas.
...
- Arquivos Parquet leves em `data/processed/`, prontos para Streamlit Community Cloud.
- Filtro aplicado na preparação dos dados para geração fotovoltaica (`UFV`).
...
O app lê diretamente os arquivos leves em `data/processed/`, então abre rápido no Streamlit Community Cloud.

## Como atualizar os dados manualmente

Para atualizar a base, baixe o Parquet público da ANEEL em:

```txt
data/raw/empreendimento-geracao-distribuida.parquet
```
...
Isso recria os arquivos leves em `data/processed/`. Não suba o arquivo bruto em `data/raw/` para o GitHub.
